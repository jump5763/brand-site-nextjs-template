...
3. 新建时从 `assets/section-template` 复制完整目录到 `src/sections/<type>.<variant>/`，填写 `section.json`、`contract.schema.json`、`definition.tsx` 和 `view.tsx`。扩展时同步更新这些文件及能力描述；保留已有内容的兼容性和原渲染默认行为，检查所有受影响的页面实例。
